#!/usr/bin/env python3
"""SysScope 2.0 — Python backend. Run: python server.py"""
...
def _excepthook(exc_type, exc_value, exc_tb):
    print("\n" + "="*52)
    print("  SYSSCOPE ERROR — please report this:")
    print("="*52)
...
print("SysScope 2.0 — gathering system info...")
_cpu   = get_cpu_name()
_gpu   = get_gpu_name()
...
CFG=os.path.join(BASE,".sysscope_config.json")
...
def api_groq_chat():
    ...
    guard = (
        "You are a PC hardware and software expert assistant embedded in SysScope, a PC monitoring app. "
...
@app.route("/api/benchmark/cpu")
def api_benchmark_cpu():
    """Time-bounded CPU benchmark — each phase runs for exactly N seconds."""
    import math, time as _t, threading
    ...
    try:
        ...
        try:
            subprocess.run([sys.executable,"-m","pip","install","numpy","-q"],timeout=30,check=True)
            import numpy as np
            HAS_NUMPY = True
        except:
            HAS_NUMPY = False
    ...
    def run_single():
        ...
        if HAS_NUMPY:
            ...
            while _t.perf_counter()-t0 < PHASE_SEC:
                C = np.dot(A,B); A = C/(C.max()+1e-9); iters+=1
        else:
            acc=0.0
            while _t.perf_counter()-t0 < PHASE_SEC:
                for i in range(1,50001): acc+=math.sqrt(i)*math.sin(i*0.001)*math.log(i)
                iters+=1
        return iters, _t.perf_counter()-t0
    ...
    def run_multi():
        ...
        def _worker(idx):
            if HAS_NUMPY:
                A=np.random.rand(400,400).astype(np.float64)
                B=np.random.rand(400,400).astype(np.float64)
                while not stop.is_set():
                    C=np.dot(A,B); A=C/(C.max()+1e-9); counts[idx]+=1
            else:
                acc=0.0
                while not stop.is_set():
                    for i in range(1,20001): acc+=math.sqrt(i)*math.cos(i*0.0007)
                    counts[idx]+=1
        threads=[threading.Thread(target=_worker,args=(i,),daemon=True) for i in range(_cores)]
        t0=_t.perf_counter()
        for t in threads: t.start()
        _t.sleep(PHASE_SEC)
        stop.set()
        for t in threads: t.join(timeout=3)
        return sum(counts), _t.perf_counter()-t0

    def run_ram():
        iters=0; t0=_t.perf_counter()
        if HAS_NUMPY:
            sz=16*1024*1024   # 16M floats = 64MB, fast enough to get multiple iters
            while _t.perf_counter()-t0 < PHASE_SEC:
                a=np.random.rand(sz).astype(np.float32)
                b=np.random.rand(sz).astype(np.float32)
                c=a*b+a-b; _=float(c.sum()); del a,b,c; iters+=1
        else:
            while _t.perf_counter()-t0 < PHASE_SEC:
                buf=bytearray(60*1024*1024); buf2=bytearray(buf); del buf,buf2; iters+=1
        return iters, _t.perf_counter()-t0

    # Run directly in this Flask thread (already its own thread with threaded=True)
    try:
        sc_i,sc_t = run_single()
        mc_i,mc_t = run_multi()
        rm_i,rm_t = run_ram()
    except Exception as e:
        return jsonify({"error":str(e)}), 500

    sc_ips = sc_i/sc_t          # 512x512 matmul/sec, single core
    mc_ips_pc = (mc_i/mc_t)/max(_cores,1)  # per-core matmul/sec
    rm_ips = rm_i/rm_t          # 64MB array ops/sec

    if HAS_NUMPY:
        # Calibration for 512x512 float64 matmul:
        # Celeron/Atom ~0.3/s→12, i3-1115G4 ~1.2/s→28, i5 ~2.5/s→45,
        # i7 ~5/s→62, i9 ~9/s→78, Ryzen9/i9-13900 ~14/s→90
        def sc_curve(x):
            if x<0.5:  return max(1,round(x*24))
            if x<2.0:  return round(12+(x-0.5)*10.7)
            if x<5.0:  return round(28+(x-2.0)*5.7)
            if x<10.0: return round(45+(x-5.0)*3.3)
            if x<16.0: return round(62+(x-10.0)*2.0)
            return min(100,round(74+(x-16.0)*1.0))
        sc_score = sc_curve(sc_ips)
        # Per-core multi uses same curve (threading overhead means it's slightly lower)
        mc_score = max(1, sc_curve(mc_ips_pc)-5)
        # RAM: 64MB array op/sec — DDR4 laptop ~1-2/s, DDR5 desktop ~4+/s
        rm_score = min(100,max(1,round(rm_ips*16+8)))
    else:
        def py_curve(x):
            if x<0.5:  return max(1,round(x*20))
            if x<2.0:  return round(10+(x-0.5)*13)
            return min(100,round(30+(x-2.0)*5))
        sc_score = py_curve(sc_ips)
        mc_score = max(1,py_curve(mc_ips_pc)-5)
        rm_score = min(100,max(1,round(rm_ips*14+10)))

    cpu_score = round(sc_score*0.45 + mc_score*0.55)

    return jsonify({
        "cpu":            min(100,cpu_score),
        "cpu_single":     min(100,sc_score),
        "cpu_multi":      min(100,mc_score),
        "ram":            min(100,rm_score),
        "single_core_ms": round(sc_t*1000/max(sc_i,1),1),
        "multi_core_ms":  round(mc_t*1000/max(mc_i,1),1),
        "ram_ms":         round(rm_t*1000/max(rm_i,1),1),
        "sc_iters":sc_i,"mc_iters":mc_i,"rm_iters":rm_i,
        "numpy":HAS_NUMPY,"cores":_cores,"threads":_thrs,
    })


@app.route("/api/benchmark")
def api_benchmark():
    # Legacy — kept so existing calls don't 404
    cpu_score = min(100, int(_cores * 12 + _thrs * 5 + _fmax * 8))
    gpu_score = [5,20,40,60,80,100][min(_tier,5)]
    ram_score = min(100, int(_ramgb * 3.2))
    scores = {
        "cpu": min(100,cpu_score), "gpu": gpu_score, "ram": ram_score,
...
if __name__ == "__main__":
    try:
        PORT=7842
        print(f"\n{'='*52}")
        print(f"  SysScope 2.0  →  http://localhost:{PORT}")
        print(f"{'='*52}\n")
        def _open():
