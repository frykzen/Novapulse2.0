...
def api_benchmark_cpu():
    """Run a real CPU benchmark — timed compute workloads."""
    import math, time as _t

    results = {}

    # ── Single-core: arithmetic + transcendental ──────────────────────────────
    N = 500_000
    t0 = _t.perf_counter()
    acc = 0.0
    for i in range(1, N+1):
        acc += math.sqrt(i) * math.sin(i * 0.001) * math.log(i)
    sc_ms = (_t.perf_counter() - t0) * 1000
    results["single_core_ms"] = round(sc_ms, 1)

    # ── Multi-core: parallel threads ──────────────────────────────────────────
    import concurrent.futures
    def _worker(n):
        s = 0.0
        for i in range(1, n+1):
            s += math.sqrt(i) * math.cos(i * 0.0007)
        return s

    chunk = 200_000
    t0 = _t.perf_counter()
    with concurrent.futures.ThreadPoolExecutor(max_workers=_cores) as ex:
        futs = [ex.submit(_worker, chunk) for _ in range(_cores)]
        [f.result() for f in futs]
    mc_ms = (_t.perf_counter() - t0) * 1000
    results["multi_core_ms"] = round(mc_ms, 1)

    # ── RAM bandwidth: large array copy ──────────────────────────────────────
    arr_size = 50 * 1024 * 1024  # 50 MB
    t0 = _t.perf_counter()
    buf = bytearray(arr_size)
    buf2 = bytearray(buf)
    ram_ms = (_t.perf_counter() - t0) * 1000
    results["ram_ms"] = round(ram_ms, 1)
    del buf, buf2

    # ── Convert to 0-100 scores ───────────────────────────────────────────────
    # Calibrated: ~1200ms single = score 30, ~200ms = score 90
    sc_score  = min(100, max(1, round(110 - (sc_ms / 14))))
    # Multi: ~800ms = 30, ~100ms = 95
    mc_score  = min(100, max(1, round(110 - (mc_ms / (_cores * 6)))))
    # RAM: <20ms = 100, >200ms = 20
    ram_score = min(100, max(1, round(110 - (ram_ms * 0.5))))
